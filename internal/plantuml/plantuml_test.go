...
func TestSanitizeSVG(t *testing.T) {
	tests := []struct {
		...
		{
			name:        "animate element stripped",
			input:       `<svg><animate attributeName="href" to="javascript:alert(1)" begin="0s"/></svg>`,
			shouldStrip: "<animate",
		},
		{
			name:        "set element stripped",
...
func TestSanitizePlantUMLCode(t *testing.T) {
	tests := []struct {
		...
		{"%file_exists", "!if %file_exists(\"/etc/passwd\")", true},
		{"%fileexists", "!assert %fileexists(\"/tmp/test\")", true},
		...
		{"safe !if", "!if $cond", false},
		{"safe !ifdef", "!ifdef MY_VAR", false},
		{"safe !ifndef", "!ifndef MY_VAR", false},
		{"safe !elseif", "!elseif $other", false},
		{"safe !else", "!else", false},
		{"safe !endif", "!endif", false},
		{"safe !while", "!while $i < 10", false},
		{"safe !endwhile", "!endwhile", false},
		{"safe !foreach", "!foreach item", false},
		{"safe !endfor", "!endfor", false},
		{"safe !return", "!return value", false},
		{"safe !assert", "!assert %strlen(x) > 0", false},
		{"safe !endfunction", "!endfunction", false},
		{"safe !endprocedure", "!endprocedure", false},
		{"unknown !directive blocked", "!newdirective foo", true},
