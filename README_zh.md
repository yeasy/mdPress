...
对你下载的文件，命令必须输出 `OK`。`--ignore-missing` 让你只用清单中的一条记录校验单个产物。

> Release 目前没有签名，macOS 二进制也尚未公证，因此 `checksums.txt` 只能防止下载损坏或被截断，无法防御被攻陷的发布。若需要更强的保证，请从源码构建。
...
就这样。你现在有了一个可打印的 PDF 和一个单文件 HTML 文档。
...
| HTML | `mdpress build --format html` | 单个 `.html` 文件，可邮件发送或上传（公式和图表从 CDN 加载，见下）|
| 站点 | `mdpress build --format site` | 多页网站，可部署到 GitHub Pages 或 Netlify |
| ePub | `mdpress build --format epub` | 电子书，支持 Kindle、Apple Books 等 |
| Typst | `mdpress build --format typst` | 基于 Typst CLI 的 PDF 后端，作为 Chromium-free 替代方案 |
| 预览 | `mdpress serve` | 本地实时预览网站 |

### HTML 和 Site 有什么区别？

- **`html`** 生成单个 `.html` 文件，所有章节在同一页面上，带侧边栏导航，图片、样式和脚本均已内联。适用于通过邮件分享或上传到文件托管平台。

  一点需要说明：如果书里含公式或 Mermaid 图表，读者打开时会从公网 CDN 加载 KaTeX 和 Mermaid。这些请求已锁定版本并做完整性校验，读者若访问不到 CDN 会看到说明文字和图表源码而不是一片空白——但页面并非完全离线，打开它会把读者的 IP 暴露给 CDN。不含公式和图表的书完全不发起网络请求。
