...
```mermaid
flowchart LR
    A["GitBook 项目"] --> B["mdpress migrate"]
    B --> C["mdpress build"]
    B --> D["mdpress serve"]
...
`migrate` 命令会将 `book.json` 转换为 `book.yaml`，并重写 GitBook 专有的模板标签（`{% hint %}`、`{% code %}`、`{% tabs %}`）：

```bash
mdpress migrate
```

使用 `--dry-run` 可在不修改文件的情况下预览变更。
...
6. **变量替换**：`mdpress migrate` 会自动重写常见的 GitBook 标签（`{% hint %}`、`{% code %}`、`{% tabs %}`）。通用的 Jinja/Nunjucks 模板（如 `{% include %}`、变量替换）不受支持；请使用纯 Markdown。
