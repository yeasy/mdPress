...
| `{author}` | The author from `book.author` | `Jane Doe` |
...
Prefer the short `{page}` / `{pages}` / `{title}` / `{author}` forms in new configs.
...
- There are no tokens for dates, versions, or chapter titles. Write such values as literal text, e.g. `left: "© 2026 — v3.2.1"`. An unrecognized `{{...}}` token is removed from the page (with a warning) rather than printed, so the text around it is left stranded.
