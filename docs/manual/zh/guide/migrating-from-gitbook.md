...
插件**不会**被转换：mdPress 没有与 GitBook 插件体系对应的机制，因此 migrate 只会把发现的
每个插件作为告警报告出来，剩下的交给你处理。
...
只有元数据会被转换过来。请注意这个文件里**没有**什么：

- 没有 `chapters:` —— mdPress 直接读取你现有的 `SUMMARY.md`，章节列表不需要再往
  `book.yaml` 里抄一遍。
- 没有插件设置。上面这次运行会打印两条告警，什么也没转换：
  ...
  语法高亮、数学公式和图表都是 mdPress 的内置能力且默认开启，所以 `highlight`、`mathjax`
  和 `mermaid` 插件本来就没有需要迁移的目标。
- 没有搜索或站点相关的键。搜索内置于 `site` 格式且始终开启；站点的公开地址是
  `output.site_url`，需要规范链接和 `sitemap.xml` 时才手动设置。

之后你自己添加的内容必须使用真正的 mdPress 键 —— `mdpress validate` 会报告无法识别的键
（`⚠ Unknown config key "title" — ignored`），而写成顶层 `title:` 而不是 `book.title`
会让书名停留在 "Untitled Book"。完整的键列表参见[配置参考](../reference/configuration.md)。
...
对于这些，迁移报告会为每一项列出一条告警，供你在运行结束后逐个处理。它不会往你的 Markdown
里插入 `TODO` 标记 —— 报告就是全部记录，请保留它。

### 审查插件告警

插件永远不会被转换。`book.json` 中的每个插件都会产生这样一行：

```text
  ! GitBook plugin "mathjax" has no direct mdpress equivalent — check the mdpress plugin registry
```

其中大多数常见插件都不需要替代品，因为 mdPress 原生支持且默认开启同样的能力：`highlight`
（语法高亮）、`mathjax`/`katex`、`mermaid` —— 参见 [Markdown 扩展](./markdown-extensions.md)
—— 以及内置于 `site` 格式的 `search`。剩下的情况参见[插件](../plugins/overview.md)：
mdPress 插件是外部可执行程序，而不是 `book.yaml` 里的条目。
