...
| 封面图片 URI 协议拦截 | P2 | 拦截 CSS url() 中的 javascript:、vbscript: 和 data: URI |
| 实时重载脚本注入修复 | P2 | 转义内联 JSON 中的 `</` 以防止 `</script>` 注入 |
...
| `generate_bookmarks: false` 被忽略 | P2 | 始终传递文档大纲选项以确保禁用书签生效 |
