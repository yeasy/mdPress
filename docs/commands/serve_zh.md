...
| `-o, --output <dir>` | `<project>/_book` | 预览站点输出目录。 |
...
- `serve` 支持 GitHub 仓库 URL。自 v0.7.12 起还支持 `--branch` 和 `--subdir`，可预览远程仓库的非默认分支或子目录。
- 文件监听会忽略生成产物（`_book/`、`*_site/`、`--output` 指定的目录以及临时交换目录），因此重建不会再次触发自身。
