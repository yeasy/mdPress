...
### Fixed
- **ePub math no longer depends on a CDN.** Formulas were rendered by KaTeX's scripts, stylesheet and fonts fetched from jsDelivr at read time, so every equation in a book required the reader to be online — something an EPUB has no business assuming and EPUB 3.3 does not permit, since a publication is meant to carry its resources. KaTeX now ships inside the book (about 600 KB, added only to books that contain a formula, MIT license included), and the math chapter's manifest entry drops the `remote-resources` property it needed for the CDN. Verified in headless Chrome with all external DNS blackholed: both an inline and a display formula render from the packaged copy, where the CDN build renders none. Readers that do not run scripts still show the LaTeX source, as before
