...
有几个文件带有版本号或发布日期，若不一起更新就会互相脱节。打 tag 发布前，请在同一个提交里全部更新：

| 文件 | 需更新的内容 |
| --- | --- |
| `cmd/root.go` | `defaultVersion` |
| `CHANGELOG.md` | 把 `[Unreleased]` 下的条目移到新版本标题下并补上日期 |
| `docs/ROADMAP.md`、`docs/ROADMAP_zh.md` | 在版本总览中标记该版本已发布，把「即将发布/规划中」小节改为已发布，并刷新 `最后更新` 行 |
| `docs/ARCHITECTURE.md`、`docs/ARCHITECTURE_zh.md` | 头部的 `版本` 与 `更新日期` 行 |
